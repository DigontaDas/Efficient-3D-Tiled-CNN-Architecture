...
| **RESULTS** | | | | |
| 24 | Baseline comparator benchmark results ($N=150$) | **Satisfied** | `stats_significance_table.md`, `unified_benchmark_comparison_table.csv`, `matched_200ep_benchmark/` | All 5 architectures converged for 200 epochs from scratch: RASNet ($0.7765$) vs nnU-Net V2 ($0.7687$), V-Net ($0.7474$, stabilized), SegResNet ($0.7469$), 3D U-Net ($0.5561$). |
| 25 | Statistical significance test results ($p$-values) | **Satisfied** | `stats_significance_table.md`, `bootstrap_CI_table.md` | RASNet vs SegResNet (Precision $p = 7.36 \times 10^{-25}$, HD95 $p = 8.72 \times 10^{-20}$, clDice $p = 8.72 \times 10^{-20}$), vs nnU-Net V2 (Precision $p = 7.36 \times 10^{-25}$, HD95 $p = 8.05 \times 10^{-10}$, ASD $p = 1.51 \times 10^{-11}$, clDice $p = 6.60 \times 10^{-09}$), vs V-Net (Precision $p = 7.36 \times 10^{-25}$, clDice $p = 4.03 \times 10^{-14}$). |
| 26 | Component-wise progressive ablation study | **Satisfied** | `Q1_Publication_Package/ablation/ablation_table.md`, `ablation_bar_chart.png` | Stepwise progression: SegResNet ($0.7469$) $\rightarrow$ AttentionGate ($0.7565$) $\rightarrow$ DeepSup ($0.7640$) $\rightarrow$ Loss ($0.7715$) $\rightarrow$ TTA ($0.7790$) $\rightarrow$ cc3d ($0.7765$). |
