...
| **RESULTS** | | | | |
| 24 | Baseline comparator benchmark results ($N=150$) | **Satisfied** | `stats_significance_table.md`, `unified_comparison_table.csv` | RASNet ($0.7862$) vs SegResNet ($0.7637$), 3D U-Net ($0.6087$), nnU-Net ($0.6003$), V-Net (diverged). |
| 25 | Statistical significance test results ($p$-values) | **Satisfied** | `stats_significance_table.md` | RASNet vs SegResNet ($p = 3.09 \times 10^{-15}$), vs 3D U-Net ($p = 2.80 \times 10^{-24}$), vs nnU-Net ($p = 8.57 \times 10^{-25}$). |
| 26 | Component-wise progressive ablation study | **Satisfied** | `Q1_Publication_Package/ablation/ablation_table.md`, `ablation_bar_chart.png` | Stepwise progression: SegResNet $\rightarrow$ AttentionGate $\rightarrow$ DeepSup $\rightarrow$ Loss $\rightarrow$ TTA $\rightarrow$ cc3d. |
...
| 39 | Model weight checkpoint availability | **Satisfied** | `results-after-hallucin-fix/checkpoints/rasnet_best.pth` | Checkpoint and training loss curves preserved. |
