...
| **RESULTS** | | | | |
| 24 | Baseline comparator benchmark results ($N=150$) | **Satisfied** | `stats_significance_table.md`, `unified_benchmark_comparison_table.csv`, `matched_200ep_benchmark/` | All 5 architectures converged for 200 epochs from scratch: RASNet ($0.7765$) vs nnU-Net V2 ($0.7687$), SegResNet ($0.7637$), V-Net ($0.7491$, stabilized), 3D U-Net ($0.6087$). |
| 25 | Statistical significance test results ($p$-values) | **Satisfied** | `stats_significance_table.md`, `bootstrap_CI_table.md` | RASNet vs SegResNet (Precision $p = 1.84 \times 10^{-18}$, clDice $p = 4.12 \times 10^{-11}$), vs nnU-Net V2 (Precision $p = 7.36 \times 10^{-25}$, HD95 $p = 9.20 \times 10^{-10}$, ASD $p = 1.26 \times 10^{-11}$, clDice $p = 9.24 \times 10^{-09}$), vs V-Net ($p = 1.31 \times 10^{-06}$). |
| 26 | Component-wise progressive ablation study | **Satisfied** | `Q1_Publication_Package/ablation/ablation_table.md`, `ablation_bar_chart.png` | Stepwise progression: SegResNet ($0.7637$) $\rightarrow$ AttentionGate $\rightarrow$ DeepSup $\rightarrow$ Loss $\rightarrow$ TTA $\rightarrow$ cc3d ($0.7765$). |
