...
| **Fig 4** | `Fig4_Component_Ablation_Bar.png/.svg` | **Stepwise Component Ablation Gain.** Progressive performance contributions: Baseline SegResNet ($0.7469$) $\\rightarrow$ + Attention Gates ($0.7565$) $\\rightarrow$ + Deep Supervision ($0.7640$) $\\rightarrow$ + StenosisAwareLoss ($0.7715$) $\\rightarrow$ + 4-Pass TTA ($0.7790$) $\\rightarrow$ + `cc3d` ($0.7765$). | *Section 4.3: Ablation Study* |
