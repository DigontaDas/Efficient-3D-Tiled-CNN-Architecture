#!/usr/bin/env python3
"""
train_vnet_200ep.py — V-Net 200-Epoch Matched Benchmark Training (Stabilized)
============================================================================
- Model: MONAI VNet (spatial_dims=3, in_channels=1, out_channels=2, act='elu', dropout_prob=0.1)
- Stability Fix: Explicit gradient clipping (max_norm=1.0) to eliminate gradient explosion
...
- Hardware: Optimized for RTX 4080 SUPER 16GB (AMP FP16, TF32, batch_size=2)
- Telemetry: Output per-epoch metrics to logs/vnet_200ep_log.csv
"""
...
def main():
    parser = argparse.ArgumentParser(description="V-Net 200-Epoch Matched Benchmark Training (Stabilized)")
    ...
    cache_dir = os.path.join(base_dir, "cache", "segresnet")  # Reuses 183GB cache: 0GB added, 0s delay
    os.makedirs(ckpt_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(cache_dir, exist_ok=True)

    best_ckpt_path = os.path.join(ckpt_dir, "vnet_best.pt")
    last_ckpt_path = os.path.join(ckpt_dir, "vnet_last.pt")
    log_file = os.path.join(log_dir, "vnet_200ep_log.csv")
    ...
    if os.path.exists(last_ckpt_path) and os.path.exists(log_file) and not args.dry_run:
        try:
            ...
            if len(existing_df) > 0 and len(existing_df) < total_epochs:
                ...
                print(f"[*] V-Net already completed {len(existing_df)} epochs. Skipping.")
                return
    ...
    print(f"[*] Model V-Net Initialized: {sum(p.numel() for p in model.parameters()):,} parameters")
    print(f"[*] Training schedule: {start_epoch} to {total_epochs} epochs | CosineAnnealingLR(T_max={total_epochs})")
    print(f"[*] Gradient clipping active: max_norm = 1.0")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        if epoch % 50 == 0:
            periodic_ckpt_path = os.path.join(ckpt_dir, f"vnet_epoch_{epoch}.pt")
    ...
    print("==================================================")
    print(f"V-Net Complete. Best Val Dice: {best_val_dice:.4f}")
