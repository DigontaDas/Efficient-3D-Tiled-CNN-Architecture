#!/usr/bin/env python3
"""
train_rasnet_200ep.py — RASNet True 0 -> 200 Epoch Matched Benchmark Training (From Scratch)
=============================================================================================
- Model: RASNet (spatial_dims=3, in_channels=1, out_channels=2, init_filters=16, dropout_prob=0.1)
- Initialization: Strict Random Initialization (NO pre-trained or historical checkpoint loaded)
- Schedule: Genuine 200 epochs with CosineAnnealingLR (T_max=200, eta_min=1e-6)
- Loss: StenosisAwareLoss (Dice + Focal, alpha=0.4, gamma=2.5) with Multi-Scale Deep Supervision
- Milestone Checkpoints: rasnet_epoch_50.pth, 100.pth, 150.pth, 200.pth, rasnet_best.pth, rasnet_last.pth
- Optimizations: val_interval=20, sw_batch_size=16 for maximum throughput on RTX 4080 SUPER 16GB
- Telemetry: Output per-epoch metrics to logs/rasnet_200ep_log.csv
"""
...
def main():
    parser = argparse.ArgumentParser(description="RASNet True 0 -> 200 Epoch Training (From Scratch)")
    ...
    parser.add_argument("--lr", type=float, default=2e-4, help="Initial learning rate for 200 schedule")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--val-interval", type=int, default=20, help="Validation interval in epochs (default: 20)")
    parser.add_argument("--sw-batch-size", type=int, default=16, help="Sliding window batch size for validation (default: 16)")
    ...
    cache_dir = os.path.join(base_dir, "cache", "segresnet")  # Shared persistent cache saves 195GB
    os.makedirs(ckpt_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(cache_dir, exist_ok=True)

    best_ckpt_path = os.path.join(ckpt_dir, "rasnet_best.pth")
    last_ckpt_path = os.path.join(ckpt_dir, "rasnet_last.pth")
    log_file = os.path.join(log_dir, "rasnet_200ep_log.csv")
    ...
    if args.dry_run:
        ...
        print(f"[*] FULL RUN: {len(train_ids)} train, {len(val_ids)} val cases, {total_epochs} epochs from scratch.")
    ...
    # Instantiate RASNet with Strict Random Initialization
    model = RASNet(
        spatial_dims=3,
        in_channels=1,
        out_channels=2,
    ...
    criterion = StenosisAwareLoss(alpha=0.4, gamma=2.5)
    ...
    # Check for existing checkpoint to resume in case of interruption
    if os.path.exists(last_ckpt_path) and os.path.exists(log_file) and not args.dry_run:
        try:
            ...
            if len(existing_df) > 0 and len(existing_df) < total_epochs:
                ...
                print(f"[*] RASNet already completed {len(existing_df)} epochs. Skipping.")
                return
        except Exception as e:
            print(f"[!] Warning: Auto-resume check encountered an issue ({e}). Initializing fresh from scratch.")
            history = []
            start_epoch = 1

    print(f"[*] Model RASNet Initialized: {sum(p.numel() for p in model.parameters()):,} parameters (Scratch / Random Init)")
    print(f"[*] Training schedule: {start_epoch} to {total_epochs} epochs | CosineAnnealingLR(T_max={total_epochs})")
    print(f"[*] Loss Function: StenosisAwareLoss (alpha=0.4, gamma=2.5) with Deep Supervision")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for batch in train_loader:
            ...
            with torch.amp.autocast("cuda", enabled=torch.cuda.is_available()):
                main_out, ds_outputs = model(images)
                ...
                total_loss = loss_main + 0.4 * aux2_loss + 0.2 * aux3_loss
        ...
        if epoch % 50 == 0:
            periodic_ckpt_path = os.path.join(ckpt_dir, f"rasnet_epoch_{epoch}.pth")
    ...
    print("==================================================")
    print(f"RASNet 200-Epoch Training Complete. Best Val Dice: {best_val_dice:.4f}")
