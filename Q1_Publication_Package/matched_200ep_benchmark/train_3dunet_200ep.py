#!/usr/bin/env python3
"""
train_3dunet_200ep.py — 3D U-Net 200-Epoch Matched Benchmark Training
====================================================================
- Model: MONAI UNet with residual units (spatial_dims=3, in_channels=1, out_channels=1, channels=(16, 32, 64, 128, 256), num_res_units=2, norm='instance', dropout=0.15)
- Schedule: Genuine 200 epochs with CosineAnnealingLR (T_max=200, eta_min=1e-6), no early exit
- Input: 128x128x128 resampled volumes
- Loss: DiceCELoss / DiceBCELoss (sigmoid=True)
- Optimizer: AdamW (lr=1e-4, weight_decay=1e-5)
- Hardware: Optimized for RTX 4080 SUPER 16GB (AMP FP16, TF32, batch_size=4)
- Telemetry: Output per-epoch metrics to logs/3dunet_200ep_log.csv
"""
...
from monai.transforms import (
    ...
    Resized, ToTensord, CropForegroundd, RandFlipd, Activations, AsDiscrete
)
...
def main():
    parser = argparse.ArgumentParser(description="3D U-Net 200-Epoch Matched Benchmark Training")
    ...
    cache_dir = os.path.join(base_dir, "cache", "3dunet")
    os.makedirs(ckpt_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(cache_dir, exist_ok=True)

    best_ckpt_path = os.path.join(ckpt_dir, "3dunet_best.pth")
    last_ckpt_path = os.path.join(ckpt_dir, "3dunet_last.pth")
    log_file = os.path.join(log_dir, "3dunet_200ep_log.csv")
    ...
    # 3D U-Net Model (ResUNet architecture matching publication benchmarks)
    model = UNet(
        spatial_dims=3,
        in_channels=1,
        out_channels=1,
    ...
    if os.path.exists(last_ckpt_path) and os.path.exists(log_file) and not args.dry_run:
        try:
            ...
            if len(existing_df) > 0 and len(existing_df) < total_epochs:
                ...
                print(f"[*] Resuming 3D U-Net from Epoch {start_epoch} -> {total_epochs} (Best Dice so far: {best_val_dice:.4f})")
            elif len(existing_df) >= total_epochs:
                print(f"[*] 3D U-Net already completed {len(existing_df)} epochs. Skipping.")
                return
    ...
    print(f"[*] Model 3D U-Net Initialized: {sum(p.numel() for p in model.parameters()):,} parameters")
        ...
        if epoch % args.val_interval == 0 or epoch == total_epochs or args.dry_run:
            ...
            with torch.no_grad():
                for v_batch in val_loader:
                    ...
                    v_outputs = [post_pred(i) for i in decollate_batch(v_logits)]
                    dice_metric(y_pred=v_outputs, y=decollate_batch(v_lbl))
            ...
            if val_dice > best_val_dice:
                ...
                torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "best_val_dice": best_val_dice}, best_ckpt_path)

        # Save Last Checkpoint Every Epoch
        torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "val_dice": val_dice}, last_ckpt_path)

        # Save Checkpoint Every 50 Epochs
        if epoch % 50 == 0:
            periodic_ckpt_path = os.path.join(ckpt_dir, f"3dunet_epoch_{epoch}.pth")
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "val_dice": val_dice}, periodic_ckpt_path)
    ...
    print("==================================================")
    print(f"3D U-Net Complete. Best Val Dice: {best_val_dice:.4f}")
